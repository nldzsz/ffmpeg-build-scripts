...
function build_ffmpeg()
{
	...
	# 编译目录不存在 则创建
    if [[ ! -d  $BUILD_DIR/build_ffmpeg_$ARH ]]; then
    	mkdir -p $BUILD_DIR/build_ffmpeg_$ARH
    fi
	...
	#    FFMPEG_CFG_CPU="--cpu=cortex-a8"
	    ...
	    echo "unknown architecture $ARH";
	    exit 1
	fi
    
    # 指定编译平台
    FFMPEG_CFG_FLAGS="$FFMPEG_CFG_FLAGS --arch=$ARH"
    # 指定编译结果存放目录
    FFMPEG_CFG_FLAGS="$FFMPEG_CFG_FLAGS --prefix==$CWD/$BUILD_THIN_DIR/ffmpeg-$ARH"
    FFMPEG_LDFLAGS="$FFMPEG_LDFLAGS $FF_XCODE_BITCODE $FF_XCRUN_OSVERSION"
	...
	$CWD/$SOURCE_DIR/ffmpeg/configure \
		$FFMPEG_CFG_FLAGS \
        --cc="$FF_XCRUN_CC" \
        $FFMPEG_CFG_CPU \
