...
#----------
set -e
#当前Linux操作系统的位数，如果是64位则填写x86_64，32位则填写x86
export FF_LINUX_ARCH="x86_64"
...
{	
	CONFIGURE_FLAGS=$1
	lib=$2
	SOURCE=$UNI_BUILD_ROOT/linux/forksource/$lib-$FF_LINUX_ARCH
	PREFIX=$UNI_BUILD_ROOT/linux/build/$lib-$FF_LINUX_ARCH
	cd $SOURCE
	
	echo ""
	echo "build linux $lib $FF_LINUX_ARCH ......."
...
do-compile-mp3lame()
{
	...
	SOURCE=./linux/forksource/mp3lame-$FF_LINUX_ARCH/include/libmp3lame.sym
...
compile_external_lib_ifneed()
{
    ...
    do
        lib=${lIBS[i]};
        FFMPEG_DEP_LIB=$UNI_BUILD_ROOT/linux/build/$lib-$FF_LINUX_ARCH/lib
		echo "$FFMPEG_DEP_LIB"
...
do-compile-ffmpeg()
{
	...
	FF_SOURCE=$FF_BUILD_ROOT/forksource/$FF_BUILD_NAME-$FF_LINUX_ARCH
	...
	# 进行裁剪
	FF_CFG_FLAGS="$FF_CFG_FLAGS --enable-ffmpeg --enable-ffprobe --enable-ffplay --enable-ffserver";
	# 开启调试;如果关闭 则注释即可
...
if [ "$FF_TARGET" == "reset" ]; then
    ...
    rm -rf linux/forksource
    . ./compile-init.sh linux
elif [ "$FF_TARGET" == "all" ]; then
    
    # 开始之前先检查fork的源代码是否存在
    if [ ! -d linux/forksource ]; then
        . ./compile-init.sh linux "offline"
    fi
    
    rm -rf linux/build/ffmpeg-*
    ...
    echo "=================="
    echo "clean ffmpeg"
    cd linux/forksource/ffmpeg && git clean -xdf && cd -
